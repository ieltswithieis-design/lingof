...
# Combine loaded dicts
all_dicts = {}
for d in [EXAMINER_PHRASES, BLOG_LEXICAL_PHRASES, DESCRIPTOR_PHRASES, TRF_IQ_PHRASES, DIRECTOR_CREATOR_PHRASES, TESTS_NAV_PHRASES]:
    for k, v in d.items():
        all_dicts[k.lower().strip()] = v

# Check which clean phrases are covered
covered = []
uncovered = []
...
with open("scripts/uncovered_clean_phrases.json", "w", encoding="utf-8") as f:
