...
export const AutonomousExaminerRoom: React.FC<AutonomousExaminerRoomProps> = ({
  ...
  const { t, currentLanguage, currentLanguageInfo, translate, speak, stopSpeaking } = useLanguage();
  ...
  const askQuestion = (qIndex: number) => {
    ...
    const localizedSpeech = localize(qText);

    speak(localizedSpeech, {
      rate: 0.93,
      onEnd: () => {
  ...
  const processAndAdvance = async (qIndex: number, audioBlob: Blob) => {
    ...
    try {
      ...
      if (nextIdx < questions.length) {
        ...
        speak(localize(`Thank you. That concludes Part ${activePart} of your speaking test.`));
      }
    }
  };
  ...
  const currentQ = questions[currentIdx] ? localize(questions[currentIdx]) : "";
            ...
            <div className="mt-4 rounded-xl border-2 border-emerald-500 bg-emerald-50/50 p-4 space-y-3">
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-800">
                ...
                <p className="italic text-slate-700 min-h-[20px]">
                  {speechTranscript ? localize(speechTranscript) : localize("Listening for your voice...")}
                </p>
              </div>
            </div>
          )}
