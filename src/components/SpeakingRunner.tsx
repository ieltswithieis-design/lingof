...
export const SpeakingRunner: React.FC<SpeakingRunnerProps> = ({ 
  ...
  const handleTranscribeAndEvaluateAudio = async () => {
    ...
    try {
      ...
      saveAttempt({
        ...
        band: data.analysis?.estimatedBand || "6.5",
      });
      setIsSaved(true);
      onComplete();
    } catch (err: any) {
  ...
  const handleAnalyzeSpeakingText = async () => {
    ...
    setIsLoadingAi(true);
    setAiError(null);
    setAiStatusMsg("Analyzing candidate transcript...");
  ...
  // Aggregated speaking band score updater
  const updateOverallSpeakingScore = (
    ...
    if (validBands.length > 0) {
      ...
      const rounded = (Math.round(avg * 2) / 2).toFixed(1);
      saveAttempt({
        section: "speaking",
        testId: test.id,
        completedAt: new Date().toISOString(),
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xs space-y-5">
          <div className="border-b border-slate-100 pb-3">
            ...
            <p className="mt-2 text-xs text-slate-500">
              The examiner will introduce themselves and ask questions on familiar topics (home, work, studies, free time).
              Click the audio icon to hear the examiner speak each question.
            </p>
          </div>
              ...
              <SpeakingQuestionRecorder
                ...
                onEvaluated={(result) => {
                  setPart1Results(prev => {
                    ...
                    updateOverallSpeakingScore(updated, part3Results);
                    return updated;
                  });
                }}
              />
      ...
      {activePart === 3 && (
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xs space-y-5">
          <div className="border-b border-slate-100 pb-3">
            ...
            <p className="mt-2 text-xs text-slate-500">
              The examiner will ask broader, abstract questions linked to the Part 2 theme.
              Aim to express and justify opinions, analyze causes and effects, and speculate about future developments.
            </p>
          </div>
              ...
              <SpeakingQuestionRecorder
                ...
                onEvaluated={(result) => {
                  setPart3Results(prev => {
                    const updated = { ...prev, [idx]: result };
                    updateOverallSpeakingScore(part1Results, updated);
                    return updated;
                  });
                }}
              />
