...
export const WritingRunner: React.FC<WritingRunnerProps> = ({ test, onBack, onComplete }) => {
  ...
  const handleSaveAttempt = () => {
    if (wordCount1 < 50 && wordCount2 < 50) {
      alert("Please write your answers for Task 1 or Task 2 before saving.");
      return;
    }

    const estimatedBand = aiAnalysis2?.estimatedBand || aiAnalysis1?.estimatedBand || "6.5";
    saveAttempt({
      section: "writing",
      testId: test.id,
